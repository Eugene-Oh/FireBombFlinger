...
/*
	shootBullet() {
		...
		if (this.elapsedTime > this.fireRate) {
			...
			if (this.jumping == -1) {
                this.game.addEntityToFrontOfList(new Bullet(gameEngine, 0, 0, this.x + 90, this.y + 35, 13, 6, true, target));
            } else {
				
                this.game.addEntityToFrontOfList(new Bullet(gameEngine, 0, 0, this.x + 90, this.y + 20, 13, 6, true, target));
            }
            this.elapsedTime = 0;
...
*/
    update() {
        ...
        if (this.game.keys["m"] && this.elapsedTime > this.fireRate) {
            ...
            if (this.jumping == -1) {
                this.game.addEntityToFrontOfList(new Bullet(gameEngine, 0, 0, this.x + 90, this.y + 35, 13, 6, true, this.direction));
            } else {
				
                this.game.addEntityToFrontOfList(new Bullet(gameEngine, 0, 0, this.x + 90, this.y + 20, 13, 6, true, this.direction));
            }
            this.elapsedTime = 0;
        } 
    ...
    draw(ctx) {
		...
		//Mouse controls
		//change direction on mouse movement
		    // draw mouse shadow
/*
    if (this.game.mouse === 0) {
        var mouse = this.game.mouse;
        ctx.save();
        ctx.globalAlpha = 0.5;
        if (this.black) {
            ctx.drawImage(ASSET_MANAGER.getAsset("./img/black.png"), mouse.x, mouse.y, 39.55, 39.55);
        } else {
            ctx.drawImage(ASSET_MANAGER.getAsset("./img/white.png"), mouse.x, mouse.y, 39.55, 39.55);
        }
        ctx.restore();
    }
*/
